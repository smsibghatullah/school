...
class StudentPayslipLine(models.Model):
    ...
    line_ids = fields.One2many("student.payslip.line.line",
        "slipline_id", "Calculations", help="Payslip line")
    ...
    company_id = fields.Many2one("res.company", "Company",
        change_default=True, default=lambda self: self.env.user.company_id,
...
class StudentFeesStructureLine(models.Model):
    ...
    line_ids = fields.One2many("student.payslip.line.line",
        "slipline1_id", "Calculations", help="Student payslip line")
    ...
    currency_id = fields.Many2one("res.currency", "Currency",
        help="Select currency")
    currency_symbol = fields.Char(related="currency_id.symbol",
        string="Symbol", help="Select currency symbol")
...
class StudentPayslip(models.Model):
    ...
    standard_id = fields.Many2one("school.standard", "Class",
        help="Select school standard")
    division_id = fields.Many2one("standard.division", "Division",
        help="Select standard division")
    medium_id = fields.Many2one("standard.medium", "Medium",
        help="Select standard medium")
    register_id = fields.Many2one("student.fees.register", "Register",
        help="Select student fee register")
    name = fields.Char("Description", help="Payslip name")
    number = fields.Char("Number", readonly=True, default=lambda self: _("/"),
        copy=False, help="Payslip number")
    ...
    type = fields.Selection([("out_invoice", "Customer Invoice"),
        ...
        change_default=True, default="out_invoice", help="Payslip type")
    company_id = fields.Many2one("res.company", "Company", required=True,
        change_default=True, readonly=True,
        default=lambda self: self.env.user.company_id, help="Related company")
...
"You can delete record in unconfirm state only!"))
    ...
    def invoice_view(self):
        ...
        for rec in self:
            invoices_rec = invoice_obj.search([("student_payslip_id", "=",
                            rec.id)])
            ...
            if len(invoices_rec) > 1:
                ...
                action["views"] = [(rec.env.ref("account.view_move_form"
                        ).id, "form")]
...
class StudentPayslipLineLine(models.Model):
    ...
    from_month = fields.Many2one("academic.month", "From Month",
        help="Academic starting month")
    to_month = fields.Many2one("academic.month", "To Month",
        help="Academic end month")
